...
function secondsToClock2(seconds)
	...
	if seconds<=0 then
		...
		return t(days).." day"..s(days)..", "..t(hours).." hour"..s(hours)..", "..t(mins).." minute"..s(mins)..", and "..t(secs).." second"..s(secs)
	end
end
...
function getSeconds(num,mod)
	...
	modifiers["d"]=86400
	modifiers["w"]=604800
	if not num or not mod then num,mod=1,3600 else num,mod=tonumber(num),modifiers[mod] end
...
local pc,err=pcall(function()
	...
	if cmd == prefix.."mute" then
		...
		local initialTime=getSeconds(arg:match(id..">%s+(%d+)(%a)"))
