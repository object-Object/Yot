...
local help={
	...
	["changelog"]={info="Show Yotogi's changelog.", usage="changelog"},
...
local helpIndex={"8ball","avatar","changelog","help","mention","roll","version","warnings"}

local staffHelp={
	["ban"]={info="Ban a user.", usage="ban <@mention> [| reason]"},
	["unban"]={info="Unban a user.", usage="unban <@mention> [| reason]"},
	["deny"]={info="Deny a user from using a bot function if they're abusing it.\nDo `"..prefix.."deny list` to list all deny keys.", usage="deny <@mention> <deny key> [| reason]"},
	["allow"]={info="Stop denying a user from using a bot function.\nDo `"..prefix.."allow list` to list all deny keys.", usage="allow <@mention> <deny key> [| reason]"},
	["idban"]={info="Ban a user by id (in case they've left the server already).\nUser IDs can be seen in the log channel.", usage="idban <id> [| reason]"},
	["idunban"]={info="Unban a user by id.\nUser IDs can be seen in the log channel.", usage="idunban <id> [| reason]"},
	["mute"]={info="Mute a user.\nTime can be specified by using one of the time codes shown in the usage. For example, to say 3 hours, type 3h.", usage="mute <@mention> [#m/#h/#d/#w] [| reason]"},
	["unmute"]={info="Unmute a user.", usage="unmute <@mention> [| reason]"},
	["staffhelp"]={info="Show the staff help menu, or show detailed info for a command.\nIn command usages, [] brackets represent an optional argument, and <> brackets represent a required argument.", usage="staffhelp [command]"},
	["shutdown"]={info="**Only usable by the server owner and bot owner.**\nRestart Yotogi in case of serious bugs.", usage="shutdown"},
	["voiceban"]={info="Ban a user from accessing voice channels.", usage="voiceban <@mention> [| reason]"},
	["voiceunban"]={info="Unban a user from accessing voice channels.", usage="voiceunban <@mention> [| reason]"},
	["warn"]={info="Warn a user. On their fourth warning, they will be banned.", usage="warn <@mention> [| reason]"},
	["unwarn"]={info="Unwarn a user.", usage="unwarn <@mention> [| reason]"}
}
local staffHelpIndex={"ban","unban","deny","allow","idban","idunban","mute","unmute","staffhelp","shutdown","voiceban","voiceunban","warn","unwarn"}
...
local pc,err=pcall(function()
	...
	if cmd == prefix.."staffhelp" then
		if not hasPermission(member,"manageRoles") then sendEmbed(channel,"Insufficient permissions.") return end
