...
local starEmojis={[minStars]="⭐️",[minStars+4]="🌟",[minStars+9]="💫"}
...
local help={
	["8ball"]={info="Consult the magic 8-ball to find the answer to your question.", usage="8ball <question>"},
	["avatar"]={info="Get the avatar of one or more users.", usage="avatar <@mention> [@mention] [@mention] [...]"},
	["changelog"]={info="Show Yotogi's changelog. Put current after the command to show only the current version's changelog.", usage="changelog [current]"},
	["help"]={info="Show the help menu, or show detailed info for a command.\nIn command usages, [] brackets represent an optional argument, and <> brackets represent a required argument.", usage="help [command]"},
	["members"]={info="Show the server's member count.", usage="members"},
	["mention"]={info="Mention a user (in case Discord won't let you). Mentions the user that best fits the search criteria.\nYou can search for part or all of their name or nickname, and/or their tag (4 digit number after their username). If you use the tag, put a # before it.", usage="mention [name or nickname][#tag]"},
	["roll"]={info="Roll a dice.", usage="roll [number of rolls]d<number of sides on dice>"},
	["version"]={info="Show Yotogi's version.", usage="version"},
	["warnings"]={info="Show your or someone else's warning level.", usage="warnings [@mention]"}
}
local helpIndex={"8ball","avatar","changelog","help","members","mention","roll","version","warnings"}

local staffHelp={
	["ban"]={info="Ban a user.", usage="ban <@mention> [| reason]", permission="banMembers"},
	["unban"]={info="Unban a user.", usage="unban <@mention> [| reason]", permission="banMembers"},
	["deny"]={info="Deny a user from using a bot function if they're abusing it.\nDo `"..prefix.."deny list` to list all deny keys.", usage="deny <@mention> <deny key> [| reason]", permission="manageRoles"},
	["allow"]={info="Stop denying a user from using a bot function.\nDo `"..prefix.."allow list` to list all deny keys.", usage="allow <@mention> <deny key> [| reason]", permission="manageRoles"},
	["banid"]={info="Ban a user by id (in case they've left the server already).\nUser IDs can be seen in the log channel.", usage="banid <id> [| reason]", permission="banMembers"},
	["unbanid"]={info="Unban a user by id.\nUser IDs can be seen in the log channel.", usage="unbanid <id> [| reason]", permission="banMembers"},
	["mute"]={info="Mute a user.\nTime can be specified by using one of the time codes shown in the usage. For example, to say 3 hours, type 3h.", usage="mute <@mention> [#m/#h/#d/#w] [| reason]", permission="manageRoles"},
	["unmute"]={info="Unmute a user.", usage="unmute <@mention> [| reason]", permission="manageRoles"},
	["staffhelp"]={info="Show the staff help menu, or show detailed info for a command.\nIn command usages, [] brackets represent an optional argument, and <> brackets represent a required argument.", usage="staffhelp [command]", permission="manageMessages"},
	["shutdown"]={info="**Only usable by the server owner and bot owner.**\nRestart Yotogi in case of serious bugs.", usage="shutdown", permission="yotogiPerms.isOwner"},
	["voiceban"]={info="Ban a user from accessing voice channels.", usage="voiceban <@mention> [| reason]", permission="manageRoles"},
	["voiceunban"]={info="Unban a user from accessing voice channels.", usage="voiceunban <@mention> [| reason]", permission="manageRoles"},
	["warn"]={info="Warn a user. On their fourth warning, they will be banned.", usage="warn <@mention> [| reason]", permission="manageRoles"},
	["unwarn"]={info="Unwarn a user.", usage="unwarn <@mention> [| reason]", permission="manageRoles"}
}
local staffHelpIndex={"ban","unban","deny","allow","banid","unbanid","mute","unmute","staffhelp","shutdown","voiceban","voiceunban","warn","unwarn"}
...
function sendEmbed(channel,text,footer_text,footer_icon)
...
function sendPermissionError(channel,command,permission)
	if not (channel and permission) then return end
	local msg=channel:send{
		embed={
			title="Insufficient permissions!",
			description="`"..prefix..command.."` requires `"..permission.."` permission.",
			color=discordia.Color.fromRGB(255,0,0).value,
		}
	}
	return msg
...
function checkPermissions(channel,member,command,permission)
	if command and staffHelp[command] then
		local permission=permission or staffHelp[command].permission
		if permission=="yotogiPerms.isOwner" then
			channel:send{
				embed={
					title="Insufficient permissions!",
					description="`"..prefix..command.."` requires bot or server ownership.",
					color=discordia.Color.fromRGB(255,0,0).value,
				}
			}
			return false
		elseif member and member:hasPermission(permission) then
			return true
		else
			if channel then
				sendPermissionError(channel,command,permission)
			end
			return false
		end
	end
					...
					if channel then
						...
						if not member then 
							...
							if tonumber(v[2])>=4 and tonumber(v[2])<100 then
								user=member.user
								table.removeKey(warnTable,k)
								sendEmbed(user:getPrivateChannel(),"You have been banned from "..guild.name.." for exceeding the warn limit.")
...
local pc,err=pcall(function()
	...
	if string.byte(reaction.emojiName)~=string.byte(starEmojis[minStars]) then return end
	...
	if reaction.count<minStars then return end
	for k,v in pairs(starEmojis) do if reaction.count>=k then emoji=v end end
	emoji=emoji or starEmojis[minStars]
	key=nil
...
local pc,err=pcall(function()
	...
	if not starChannel then return end
	if channel==starChannel then return end
	for msgReaction in message.reactions:iter() do if type(msgReaction.emojiName)=="string" and string.byte(msgReaction.emojiName)==string.byte(starEmojis[minStars]) then reaction=msgReaction end end
	if not reaction or reaction.count<minStars then return end
	for k,v in pairs(starEmojis) do if reaction.count>=k then emoji=v end end
	emoji=emoji or starEmojis[minStars]
	key=nil
	for k,v in ipairs(starTable) do local temp=table.keyByValue(v,message.id) if temp then key=k end end
...
local pc,err=pcall(function()
	...
	if key then
		...
		temp=nil
		for msgReaction in message.reactions:iter() do if string.byte(msgReaction.emojiName)==string.byte(starEmojis[minStars]) then temp=msgReaction end end
		if not temp or reaction.count<minStars then 
			sbMessage:delete()
...
local pc,err=pcall(function()
	...
	if message.content:lower():match("discord.gg") then
		if hasPermission(member,"manageMessages") then return end
		local text=message.content:lower():gsub("discord.gg%/etqt2xg","")
	...
	if message.content:lower():match("youtube.com/watch%?v=%S") then
		...
		if channelID then
			local res,body=http.request("GET","https://www.googleapis.com/youtube/v3/channels?id="..channelID.."&part=snippet&key="..youtubeAPIKey)
			local authorName=body:match('"title": (%b"")'):gsub('"',""):lower()
			if authorName then
				if authorName:match("marmarchan") then return end
	...
	if cmd == prefix.."help" then
		if arg and help[arg:gsub("^%"..prefix,""):lower()] then
			local helpEntry=help[arg:gsub("^%"..prefix,""):lower()]
			sendEmbed(channel,"**Help for** `"..prefix..arg:gsub("^%"..prefix,""):lower().."`**:**\n"..helpEntry.info,"Usage: "..prefix..helpEntry.usage)
		else
			local helpOutput="**Commands:**```\n"
			for _,command in ipairs(helpIndex) do
				helpOutput=helpOutput..prefix..command.."\n"
			end
			helpOutput=helpOutput:gsub("\n$","").."```"
			sendEmbed(channel,helpOutput,"Do "..prefix.."help [command] for more info on a command.")
		end
		return
	end

	if cmd == prefix.."staffhelp" then
		if not checkPermissions(channel,member,"staffhelp") then return end
		if arg and staffHelp[arg:gsub("^%"..prefix,""):lower()] then
			local helpEntry=staffHelp[arg:gsub("^%"..prefix,""):lower()]
			sendEmbed(channel,"**Help for** `"..prefix..arg:gsub("^%"..prefix,""):lower().."`**:**\n"..helpEntry.info,"Usage: "..prefix..helpEntry.usage.." -- Required permission: "..helpEntry.permission)
		else
			local helpOutput="**Staff commands:**```\n"
			for _,command in ipairs(staffHelpIndex) do
				helpOutput=helpOutput..prefix..command.."\n"
			end
			helpOutput=helpOutput:gsub("\n$","").."```"
			sendEmbed(channel,helpOutput,"Do "..prefix.."staffhelp [command] for more info on a command.")
		end
		return
	end
	...
	if cmd == prefix.."members" then
		local memberCount=#guild.members
		sendEmbed(channel,"Current member count: `"..memberCount.."`")
		return
	end

	if cmd == prefix.."avatar" then
		if not message.mentionedUsers.first then
			sendEmbed(channel,"User(s) not found.")
			return
		end
		local output=""
	...
	if cmd == prefix.."8ball" then
		if isDenied(member,"8ball") then return end
		if not arg then sendEmbed(channel,"No question found.") return end
		local question="**"..arg.."**\n"
		...
		channel:send{
			embed={
				...
				description=question..":8ball: "..response,
				color=embedColor
			}
		}
		return
	...
	if cmd == prefix.."deny" then
		if not checkPermissions(channel,member,"deny") then return end
	...
	if cmd == prefix.."allow" then
		if not checkPermissions(channel,member,"allow") then return end
		if not arg then sendUsage(message.author,cmd,"<mention> <deny key>") return end
		if argTable[1]=="list" then
			local list="**Denies**"
			for k,v in pairs(denies) do
	...
	if cmd == prefix.."banid" then
		if not checkPermissions(channel,member,"banid") then return end
	...
	if cmd == prefix.."unbanid" then
		if not checkPermissions(channel,member,"unbanid") then return end
		if not arg then sendUsage(message.author,cmd,"<user id>") return end
		local id=arg:match("%d+")
		if not id then return end
		local user=client:getUser(id)
	...
	if cmd == prefix.."voiceban" then
		if not checkPermissions(channel,member,"voiceban") then return end
	...
	if cmd == prefix.."voiceunban" then
		if not checkPermissions(channel,member,"voiceunban") then return end
		if not arg then sendUsage(message.author,cmd,"<mention>") return end
		local id=arg:match("%D+(%d+)%D")
		if not id then return end
		local vbMember = guild:getMember(id)
	...
	if cmd == prefix.."ban" then
		if not checkPermissions(channel,member,"ban") then return end
		if not arg then sendUsage(message.author,cmd,"<mention>") return end
		local id=arg:match("%D+(%d+)%D")
		if not id then return end
		local user=client:getUser(id)
		...
		if reason then reason=" (Reason: "..reason..")" else reason="" end
		local banMessage
		if guild:getMember(id) then banMessage=sendEmbed(user,"You have been banned from "..guild.name.."."..reason) end
		if guild:banUser(user) then
			sendEmbed(channel,user.name.." has been banned."..reason)
		else
			sendEmbed(channel,user.name.." could not be banned.")
			if banMessage then banMessage:delete() end
		end
		local logChannel = get(guild.textChannels,"nsfw-lurkchannel")
	...
	if cmd == prefix.."unban" then
		if not checkPermissions(channel,member,"unban") then return end
		if not arg then sendUsage(message.author,cmd,"<mention>") return end
		local id=arg:match("%D+(%d+)%D")
		if not id then return end
		local user=client:getUser(id)
	...
	if cmd == prefix.."mute" then
		if not checkPermissions(channel,member,"mute") then return end
	...
	if cmd == prefix.."unmute" then
		if not checkPermissions(channel,member,"unmute") then return end
		if not arg then sendUsage(message.author,cmd,"<mention>") return end
		local id=arg:match("%D+(%d+)%D")
		if not id then return end
		local silent=arg:match("%-%-silent")
	...
	if cmd == prefix.."warn" then
		if not checkPermissions(channel,member,"warn") then return end
		if not arg then sendUsage(message.author,cmd,"<mention>") return end
		local id=arg:match("%D+(%d+)%D")
		if not id then return end
		local silent=arg:match("%-%-silent")
		...
		if entry[2]>=4 and entry[2]<100 then
			if not checkPermissions(channel,member,"warn","banMembers") then
				entry[2]=entry[2]-1
			else
				table.removeKey(warnTable,id)
				sendEmbed(channel,warnUser.name.." has been banned for exceeding the warn limit."..reason)
				sendEmbed(warnUser:getPrivateChannel(),"You have been banned from "..guild.name.." for exceeding the warn limit."..reason)
				warnMember:ban()
				local logChannel = get(guild.textChannels,"nsfw-lurkchannel")
	...
	if cmd == prefix.."unwarn" then
		if not checkPermissions(channel,member,"unwarn") then return end
		if not arg then sendUsage(message.author,cmd,"<mention>") return end
		local id=arg:match("%D+(%d+)%D")
		if not id then return end
		local silent=arg:match("%-%-silent")
