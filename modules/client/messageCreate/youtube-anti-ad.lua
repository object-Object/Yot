		...
		if doKick then
			...
			if kickValid then
				...
				if not success then
					...
					local text = f(lang.warn.user_kicked_auto, name, entry.level)..reason
		...
		elseif doBan then
			...
			if banValid then
				...
				if not success then
					...
					local text = f(lang.warn.user_banned_auto, name, entry.level)..reason
					utils.sendEmbed(message.channel, text, "ffff00")
					utils.sendEmbedSafe(staffLogChannel, text, "ffff00", responsibleName)
					return
				end
				...
				utils.sendEmbed(message.channel, f(lang.pl(lang.error.ban_auto_warn_fail, entry.level), name, entry.level, banErr), "ff0000")
			end
		end
